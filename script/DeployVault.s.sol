...
/// @notice Deploy and configure a morpho v2 vault using toml files in config
/// 1) Ensure BROADCASTER_KEY, BROADCASTER, and CONFIG_DIR are set in .env
/// 2) Run `FOUNDRY_PROFILE=production forge script script/DeployVault.s.sol $CONFIG_DIR --rpc-url {rpc_url} --private-key $BROADCASTER_KEY --verify --verifier etherscan --verifier-api-key $ETHERSCAN_API_KEY --slow
contract DeployVault is BaseScript, Config {
    string constant DEFAULT_DIR = "./config/mainnet/tgld/vaults/tgld_usdc_vault.toml";
    ...
    /// @notice Parameters loaded from toml file
    VaultParams params;
    ...
    function deploy() internal broadcast returns (DeploymentResult memory deployment) {
        ...
        // load factory addresses based on chain
        if (block.chainid == 1) {
            ...
            params.initial_vault = address(new MockERC4626("Steakhouse USDC", "SUSDC", IERC20(params.asset)));
            // perform initial vault dead deposit
            IERC20(params.asset).approve(params.initial_vault, 1e6);
            IERC4626(params.initial_vault).deposit(1e6, address(0xDEAD));
        } else {
            revert("chain not supported for vault deployment");
        }
