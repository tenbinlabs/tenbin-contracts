...
contract TenbinIntegrationTest is BaseTest {
    ...
    function test_Mint_Swap_Deposit() public {
        ...
        vm.prank(admin);
        manager.setSwapTolerance(address(collateral), address(collateral2), 0);
        ...
        vm.prank(multicaller);
        // TODO: Find a way to expect emits from multiple addresses in a multicall (ENG-234)
        vm.expectEmit();
    ...
    function test_Reward_Staking(uint256 amount) public {
        ...
        // manager generates revenue and it gets pulled by revenue module
        pullFunds(amount);
        ...
        // create mint order
        uint256 assetAmount = amount * 3e8;
        IController.Order memory order = getMintOrder(collateral, amount, assetAmount, 0);
