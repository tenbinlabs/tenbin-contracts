...
contract DeploymentTest is Test, Config {
    ...
    function test_Deployment() public {
        ...
        assertEq(deployment.manager.hasRole(COLLECTOR_ROLE, config.get("collector").toAddress()), true);
        ...
        assertEq(deployment.revenueModule.hasRole(DEFAULT_ADMIN_ROLE, config.get("owner").toAddress()), true);
        assertEq(deployment.revenueModule.hasRole(KEEPER_ROLE, config.get("keeper").toAddress()), true);
        ...
        if (block.chainid != 31337) {
            ...
            assertFalse(deployment.manager.hasRole(COLLECTOR_ROLE, config.get("deployer").toAddress()));
            ...
            assertFalse(deployment.revenueModule.hasRole(KEEPER_ROLE, config.get("deployer").toAddress()));
