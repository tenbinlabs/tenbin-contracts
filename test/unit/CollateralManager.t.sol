...
import {Mock1InchRouterWithInsufficientAmountSent} from "test/mocks/Mock1InchRouter.sol";
import {MockERC20} from "test/mocks/MockERC20.sol";
import {MockERC4626} from "test/mocks/MockERC4626.sol";
import {MockEtherReceiver} from "test/mocks/MockEtherReceiver.sol";
...
contract CollateralManagerTest is BaseTest {
    ...
    function test_CollateralManager_UpgradeToAndCall() public {
        ...
        assertEq(manager.hasRole(COLLECTOR_ROLE, collector), true);
    ...
    function test_Deposit_Withdraw_Revenue() public {
        ...
        assertEq(manager.vaults(address(collateral)).totalAssets(), 0);
    ...
    function test_Revert_AccessControl() public {
        ...
        vm.expectPartialRevert(IAccessControl.AccessControlUnauthorizedAccount.selector);
        manager.setSwapTolerance(address(0), address(0), 0);
    ...
    function test_GetRevenue() public {
        ...
        assertEq(manager.getRevenue(address(collateral)), 10000e6);
    }
    ...
    function test_GetRevenue_NegativeTotalValue() public {
        ...
        // revenue does not change when a loss is incurred
        assertEq(manager.getRevenue(address(collateral)), 10000e6);
    }
    ...
    function test_Revert_WithdrawRevenue() public {
        // only collector can withdraw revenue
        vm.expectPartialRevert(IAccessControl.AccessControlUnauthorizedAccount.selector);
    ...
    function test_WithdrawRevenue() public {
        ...
        assertEq(manager.pendingRevenue(address(collateral)), 500e6);
        assertEq(collateral.balanceOf(collector), 500e6);
        ...
        assertEq(collateral.balanceOf(collector), 1000e6);
    ...
    function test_Revert_SwapCollateral() public {
        ...
        vm.prank(admin);
        manager.setSwapTolerance(address(collateral), address(collateral2), ctx.bps); // 1%
        ...
        vm.expectRevert(ICollateralManager.InvalidSlippage.selector);
        manager.swap(abi.encode(params), swapData);
        ...
        vm.startPrank(curator);
        vm.expectRevert(ICollateralManager.InvalidSwapAmount.selector);
        manager.swap(abi.encode(params), swapData);

        // invalid destination token cap
        vm.stopPrank();
        vm.prank(capAdjuster);
        manager.setSwapCap(address(collateral), params.amount);
    ...
    function test_SwapCollateral() public {
        ...
        manager.addCollateral(address(collateral2), address(vault2));
        vm.prank(admin);
        manager.setSwapTolerance(address(collateral), address(collateral2), 100); // 1%
        (
            bytes memory parameters,
            bytes memory swapData,
            IAggregationRouterV6.SwapDescription memory desc,
        ...
        vm.stopPrank();
        vm.prank(admin);
        manager.setSwapTolerance(address(token1), address(token2), 100); // 1%
    ...
    function test_SwapCollateralSlippage() public {
        ...
        vm.stopPrank();
        vm.prank(admin);
        manager.setSwapTolerance(address(ctx.token1), address(ctx.token2), 100); // 1%
    ...
    function test_Revert_UpdateController() public {
        ...
        vm.expectPartialRevert(IAccessControl.AccessControlUnauthorizedAccount.selector);
        manager.updateController(address(newController), collaterals);

        // revert if new controller is zero address
        vm.prank(owner);
        vm.expectRevert(ICollateralManager.NonZeroAddress.selector);
        manager.updateController(address(0), collaterals);

        // revert when collateral not supported
        vm.prank(owner);
        vm.expectRevert(ICollateralManager.CollateralNotSupported.selector);
        manager.updateController(address(newController), collaterals);
    }
    ...
    function test_setSwapTolerance() public {
        vm.prank(admin);
        vm.expectEmit();
        emit ICollateralManager.SwapToleranceUpdated(address(collateral), address(collateral2), 1);
        manager.setSwapTolerance(address(collateral), address(collateral2), 1);

        assertEq(manager.swapTolerance(address(collateral), address(collateral2)), 1);
    }
    ...
    function test_fuzz_Slippage(uint256 bps, uint8 dec1, uint8 dec2, uint256 amount) public {
        SwapContext memory ctx;
        ctx.dec1 = uint8(bound(dec1, 6, 20));
        ctx.dec2 = uint8(bound(dec2, 6, 20));
        ctx.bps = bound(bps, 50, 5_000); //Avoid high slippage
        ...
        vm.stopPrank();
        vm.prank(admin);
        manager.setSwapTolerance(address(ctx.token1), address(ctx.token2), ctx.bps);
        ...
        uint256 limitDiff = (params.minReturnAmount * ctx.bps / 10_000);
    ...
    function test_SwapReentrancy() public {
        ...
        manager.addCollateral(address(collateral2), address(vault2));
        vm.prank(admin);
        manager.setSwapTolerance(address(badCollateral), address(collateral2), 100); // 1%
        (
            bytes memory parameters,
            bytes memory swapData,
            /* IAggregationRouterV6.SwapDescription memory  desc*/,
