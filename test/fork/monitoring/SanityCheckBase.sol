...
/// @notice General assumptions that must always hold to ensure a healthy state
contract SanityCheckBase is MonitorBase {
    // Asset Token
    ...
    // `distributor` is correctly set
    function testDistributorAddress() public view {
        if (contracts.collateral_manager != TGLD_MANAGER) {
            assertEq(CollateralManager(contracts.collateral_manager).distributor(), DISTRIBUTOR);
        }
    }

    // claim recipient is set in distributor for contract
    function testClaimRecipient() public {
        // tGLD collateral manager should not be checked
        if (contracts.collateral_manager != TGLD_MANAGER) {
            assertEq(
                IDistributor(DISTRIBUTOR).claimRecipient(contracts.collateral_manager, address(0)), REWARD_RECIPIENT
            );
        }
    }

    // Controller
    // Current mint limit cannot exceed max limit
    function testMintLimit() public view {
        (, uint256 amount) = Controller(contracts.controller).mintLimit();
        assertGe(Controller(contracts.controller).blockMintLimit(), amount);
    }

    // Current redeem limit cannot exceed max limit
    function testRedeemLimit() public view {
        (, uint256 amount) = Controller(contracts.controller).redeemLimit();
        assertGe(Controller(contracts.controller).blockRedeemLimit(), amount);
    }
    ...
    // Controller saved `stakedAsset` should be linked to asset
    function testControllerStakedAssetCorrectness() public view {
        address controllerStakedAsset = Controller(contracts.controller).stakedAsset();
        assertEq(StakedAsset(controllerStakedAsset).asset(), Controller(contracts.controller).asset());
    }
    ...
    function testStakingSupply() public view {
        if (contracts.asset_token == 0x6a547b25534234bb79CE6961a23Db13DE154b6F4) {
