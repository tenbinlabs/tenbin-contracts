...
contract OneInchForkTest is ForkBaseTest {
    ...
    function setUp() public override {
        ...
        // deal usdc and usdt to manager
        dealTo(usdc, managerAddress, 50e6);
        dealTo(usdt, managerAddress, 50e6);
    }
    ...
    function testFork_1Inch_Setup() public view {
        assertGt(usdc.balanceOf(managerAddress), 40e6);
        assertGt(usdt.balanceOf(managerAddress), 40e6);
    }

    function testFork_1Inch_Swap() public {
        // save balances before swap
        uint256 usdcBalanceBefore = usdc.balanceOf(managerAddress);
        uint256 usdtBalanceBefore = usdt.balanceOf(managerAddress);
        ...
        // update dstReceiver address
        desc.dstReceiver = payable(managerAddress);
        ...
        // check balances
        assertApproxEqAbs(usdc.balanceOf(managerAddress), usdcBalanceBefore - 40e6, SWAP_TOLERANCE_USDC);
        assertGt(usdt.balanceOf(managerAddress), usdtBalanceBefore + 40e6 - SWAP_TOLERANCE_USDT);
    }
}
