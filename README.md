...
Roles and existing deployments are tracked in deployments.toml. When running the deployment script, a file is created in the broadcast called `deployments.json` containing the deployed contracts and roles.
...
1) Set `deployer` is deployments.toml. You can configure additional accounts here (see [31337.address]).
